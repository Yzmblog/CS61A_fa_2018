    ...
    def reduce_armor(self, amount):
        ...
        if self.armor<=0:
            ...
            for i in range(len(copy)):
                copy[i].armor-=self.damage
                
                if copy[i].armor<=0:
                    self.place.bees.remove(self.place.bees[i-re])
                    re+=1
        # END Problem 5
...
class BodyguardAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 9
        "*** YOUR CODE HERE ***"
        return ((self.contained_ant==None) and (self.is_container==True) and (other.is_container==False)) 
        # END Problem 9
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.is_queen:
            attack_bee=self.nearest_bee(colony.hive)
            self.throw_at(attack_bee)
            place=self.place
            while place is not None:
                if place.exit is not None and place.exit.ant is not None :
                    if place.exit.ant.is_container:
                        if not place.exit.ant.is_double:
                            place.exit.ant.damage*=2
                            place.exit.ant.is_double=True
                        if place.exit.ant.contained_ant is not None and not place.exit.ant.contained_ant.is_double:
                            place.exit.ant.contained_ant.damage*=2
                            place.exit.ant.contained_ant.is_double=True
                    elif (not place.exit.ant.is_double):
                        place.exit.ant.damage*=2
                        place.exit.ant.is_double=True
                place=place.exit
        else:
