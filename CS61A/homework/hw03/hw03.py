...
def pingpong(n):
    ...
    if n<=7:
        return n
    elif num_sevens(n-1)!=0 or (n-1)%7==0:
...
def count_change(amount):
    ...
    """
    "*** YOUR CODE HERE ***"
    def count_constrained(amount, smallest_coin):
        if amount==0:
            return 1
        if smallest_coin>amount:
            return 0
        without_coin=count_constrained(amount, smallest_coin*2)
        with_coin=count_constrained(amount-smallest_coin, smallest_coin)
        return without_coin+with_coin
    return count_constrained(amount, 1)#??
...
def flatten(lst):
   ...
   ## else:
     ##   r=lst[len(lst)-1]
       ## if type(r)==list:
         ##   return flatten(lst[:len(lst)-1])+flatten(r)
        ##else:
          ##  return flatten(lst[:len(lst)-1])+[r]
    if not lst:
        return []
