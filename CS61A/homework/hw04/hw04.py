...
def make_withdraw(balance, password):
    ...
    def withdraw(amount,given_password):
        nonlocal attempt_count
        
        nonlocal balance
        if attempt_count<3:
            if given_password!=password:
                s.append(given_password)
                attempt_count+=1
            ...
            return "Your account is locked. Attempts: "+str(s)
    return withdraw
...
def make_joint(withdraw, old_password, new_password):
    ...
    if type(error)==str:
        return error
    def withdraw_return(amount,password):
        if password==new_password:
            return withdraw(amount,old_password)
        else:
            return withdraw(amount,password)
    return withdraw_return
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    "*** YOUR CODE HERE ***"
    assert not (lower_bound(y)<=0<=upperbound(y))
...
def quadratic(x, a, b, c):
    ...
    """
    "*** YOUR CODE HERE ***"
    x1=a*lower_bound(x)*lower_bound(x)+b*lower_bound(x)+c
    x2=a*upper_bound(x)*upper_bound(x)+b*upper_bound(x)+c
    if a!=0:
        x3=b*b/(4*a)-b*b/(2*a)+c
        if lower_bound(x)<(-b/(2*a))<upper_bound(x):  
            return interval(min(x1,x2,x3),max(x1,x2,x3)) 
        else:
